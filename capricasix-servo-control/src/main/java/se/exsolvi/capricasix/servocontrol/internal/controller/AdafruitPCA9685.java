...
public class AdafruitPCA9685 {
  ...
  public void setPWM(int channel, int on, int off) throws IllegalArgumentException {
    ...
    if (on < 0 || on > 4095) {
      throw new IllegalArgumentException("On must be in [0, 4095]: " + on);
    }
    if (off < 0 || off > 4095) {
      throw new IllegalArgumentException("Off must be in [0, 4095]: " + off);
    }
    if (on > off) {
      throw new IllegalArgumentException("Off must be greater than On: Off: " + off + " on: " + on);
    }
    try {
      ...
      //System.out.println("Time to write: " + (System.currentTimeMillis() - start));
    } catch (IOException ioe) {
      ioe.printStackTrace();
    }
  }
  ...
  public void setServoPulse(int channel, float pulseMS) {
    ...
    pulse /= pulseLength;
    //log.debug("{} us per bit, pulse: {}, inparam: {}", pulseLength, pulse, pulseMS);
